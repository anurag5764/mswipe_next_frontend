...
const BenefitOfMerchantaPortal = ({ acordianData }) => {
  ...
  const prefix = acordianData.prefix || "Benefits of ";
  const emphasis = acordianData.emphasized || "merchant app";

  const data = acordianData.data || benefitData;
  return (
    <>
          ...
          <div className="row sp-bg mt-2 mt-lg-5 align-items-center">
            ...
            <div className="col-lg-6">
              <Accordion className="accordion-04" defaultActiveKey="0">
                {data.map((item, index) => (
                  <Accordion.Item eventKey={index.toString()} key={index} onClick={() => setImage(item.image)}>
